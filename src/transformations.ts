 ...
 */
export function estimateTokens(input: Message[] | Message | string): number {
    let text: string;
 ...
 */
export function estimateTokensWithRoles(messages: Message[]): number {
    let totalTokens = 0;
 ...
 */
export function createStreamingTransformer(model: string, requestId?: string) {
    const id = requestId || `chatcmpl-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    const created = Math.floor(Date.now() / 1000);
    
    return function transformStreamChunk(content: string, isLast: boolean = false) {
        const choice: any = {
            index: 0,
 ...
 */
export function validateTransformationRequest(request: ChatCompletionRequest): string[] {
    const errors: string[] = [];
 ...
 */
export function transformError(error: Error, model?: string, provider?: string) {
    const message = error.message || 'An unknown error occurred';
    
    // Determine error type based on error message patterns (case insensitive)
    let type: 'invalid_request_error' | 'authentication_error' | 'api_error' | 'rate_limit_error' = 'api_error';
