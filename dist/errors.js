...
exports.ValidationErrorHandler = exports.ErrorResponseFormatter = exports.KeyRotationErrorHandler = exports.ErrorTransformer = void 0;
...
class ErrorTransformer {
     ...
     */
    static fromGenericError(error, context) {
        const message = error.message || 'Unknown error occurred';
        // Check for specific error patterns
        if (message.toLowerCase().includes('rate') || message.toLowerCase().includes('quota') || message.includes('429')) {
            return types_1.ApiError.rateLimit(context?.provider || 'unknown');
        }
        if (message.includes('authentication') || message.includes('401') || message.includes('unauthorized')) {
            return types_1.ApiError.authentication(message, context?.provider);
        }
        if (message.includes('timeout') || message.includes('ETIMEDOUT')) {
            return types_1.ApiError.timeout(context?.provider || 'unknown', 30000);
        }
        if (message.includes('model') && message.includes('not found')) {
            return types_1.ApiError.modelNotFound(context?.model || 'unknown');
        }
        if (message.includes('All') && message.includes('failed')) {
            // This is likely a key exhaustion error from LlmManager
            return new types_1.ApiError(message, 'api_error', 'keys_exhausted', 502, {
        ...
        return new types_1.ApiError(message, 'server_error', 'internal_error', 500, {
            timestamp: new Date().toISOString(),
            lastError: message
        });
    }
     ...
     */
    static fromValidationError(message, fieldErrors) {
        return types_1.ApiError.validation(message, fieldErrors);
    }
    /**
    ...
    static fromProviderError(error, provider) {
        ...
        if (message.includes('insufficient_quota') || message.includes('quota')) {
            return new types_1.ApiError(`Insufficient quota for ${provider}`, 'api_error', 'insufficient_quota', 429, { provider, lastError: message, timestamp: new Date().toISOString() });
        }
        if (message.includes('model_not_found') || message.includes('model not found')) {
            return new types_1.ApiError(`Model not found on ${provider}`, 'invalid_request_error', 'model_not_found', 400, { provider, lastError: message });
        }
        // Default provider error
        return new types_1.ApiError(`Provider ${provider} error: ${message}`, 'api_error', 'provider_error', 502, {
            provider,
            lastError: message,
            timestamp: new Date().toISOString()
        });
 ...
 */
class KeyRotationErrorHandler {
    /**
     * Handle key exhaustion errors with detailed status information
     */
    static handleKeyExhaustion(provider, error, llmManager) {
        const keyStatuses = llmManager.apiKeyStatus[provider] || [];
        return new types_1.ApiError(`All ${provider} API keys failed. Last error: ${error.message}`, 'api_error', 'keys_exhausted', 502, {
            provider,
            keyStatuses,
     ...
     */
    static handleKeyFailure(provider, keyIndex, error, totalKeys) {
        const isRateLimit = error.message && (error.message.toLowerCase().includes('rate') ||
            error.message.toLowerCase().includes('quota') ||
            error.message.includes('429'));
        if (isRateLimit) {
            return new types_1.ApiError(`API key ${keyIndex + 1}/${totalKeys} for ${provider} is rate limited`, 'rate_limit_error', 'rate_limited', 429, {
                provider,
                lastError: error.message,
                timestamp: new Date().toISOString()
            });
        }
        return new types_1.ApiError(`API key ${keyIndex + 1}/${totalKeys} for ${provider} failed: ${error.message}`, 'authentication_error', 'invalid_api_key', 401, {
            provider,
            lastError: error.message,
            timestamp: new Date().toISOString()
        });
     ...
     */
    static getKeyStatusDetails(llmManager, provider) {
        const keyStatuses = llmManager.apiKeyStatus[provider] || [];
        return {
            provider,
            keyStatuses,
...
exports.KeyRotationErrorHandler = KeyRotationErrorHandler;
/**
...
class ErrorResponseFormatter {
     ...
     */
    static formatHttpResponse(error) {
        return {
    ...
    static formatMultipleErrors(errors) {
        if (errors.length === 0) {
            return this.formatHttpResponse(new types_1.ApiError('Unknown error occurred', 'server_error', 'internal_error', 500));
        }
        if (errors.length === 1) {
        ...
        const combinedError = new types_1.ApiError(combinedMessage, primaryError.type, primaryError.code, highestStatusCode, {
     ...
     */
    static createSafeErrorResponse(error, includeDetails = false) {
     ...
     */
    static logAndFormatError(error, requestId, includeDetailsInResponse = false) {
...
class ValidationErrorHandler {
    ...
    static validateRequiredFields(data, requiredFields) {
        ...
        if (missingFields.length > 0) {
            return types_1.ApiError.validation(`Missing required fields: ${missingFields.join(', ')}`, fieldErrors);
        }
        return null;
    }
    /**
    ...
    static validateModel(model) {
        if (!model || typeof model !== 'string') {
            return types_1.ApiError.validation('Model must be a non-empty string', {
                model: 'Model is required and must be a string'
            });
        }
        if (model.length > 100) {
            return types_1.ApiError.validation('Model name is too long', {
    ...
    static validateTemperature(temperature) {
        if (temperature !== undefined) {
            if (typeof temperature !== 'number' || isNaN(temperature)) {
                return types_1.ApiError.validation('Temperature must be a number', {
                    temperature: 'Temperature must be a valid number'
                });
            }
            if (temperature < 0 || temperature > 2) {
                return types_1.ApiError.validation('Temperature must be between 0 and 2', {
    ...
    static validateMaxTokens(maxTokens) {
        if (maxTokens !== undefined) {
            if (typeof maxTokens !== 'number' || isNaN(maxTokens) || !Number.isInteger(maxTokens)) {
                return types_1.ApiError.validation('max_tokens must be an integer', {
                    max_tokens: 'max_tokens must be a valid integer'
                });
            }
            if (maxTokens < 1 || maxTokens > 32000) {
                return types_1.ApiError.validation('max_tokens must be between 1 and 32000', {
     ...
     */
    static validateTopP(topP) {
        if (topP !== undefined) {
            if (typeof topP !== 'number' || isNaN(topP)) {
                return types_1.ApiError.validation('top_p must be a number', {
                    top_p: 'top_p must be a valid number'
                });
            }
            if (topP < 0 || topP > 1) {
                return types_1.ApiError.validation('top_p must be between 0 and 1', {
    ...
    static validateMessages(messages) {
        if (!Array.isArray(messages)) {
            return types_1.ApiError.validation('Messages must be an array', {
                messages: 'Messages must be an array'
            });
        }
        if (messages.length === 0) {
            return types_1.ApiError.validation('Messages array cannot be empty', {
        ...
        if (Object.keys(fieldErrors).length > 0) {
            return types_1.ApiError.validation('Invalid messages format', fieldErrors);
        }
        return null;
    }
}
